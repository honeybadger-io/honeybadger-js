...
- **Examples are non-build artifacts.** `packages/*/examples/` directories are documentation; they are not built by the package build, are mostly excluded from lint, and shouldn't import from the package's `dist/` — they should consume the published API. They are also excluded from lerna's change detection by the `ignoreChanges` globs in `lerna.json`; without those, a dependency bump inside an example would version and publish the owning package and its dependents. Don't remove them.
