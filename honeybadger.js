...
Notice = (function() {
  function Notice(options) {
    ...
    this.stackInfo = this.options.stackInfo || (this.error && Honeybadger.TraceKit.computeStackTrace(this.error));
...
Honeybadger = {
  ...
  configure: function(options) {
    ...
    (_base = this.configuration)[app] || (_base[app] = new Configuration());
    ...
    for (k in options) {
      v = options[k];
      this.configuration[app][k] = v;
    }
    if (this.configured === false) {
      this.configured = true;
      this.configuration["default"] = this.configuration[app];
    }
    this.TraceKit.collectWindowErrors = this.configuration[app].onerror;
    return this;
  },
  configuration: {
  ...
  notify: function(error, app, options) {
    var handler, k, notice, v, _i, _len, _ref;
    if (options == null) {
      options = {};
    }
    if (!this.configured || this.configuration["default"].disabled === true) {
      return false;
    }
    ...
    return this._sendRequest(notice.toJSON(), app);
  },
  wrap: function(func, app) {
    app || (app = 'default');
    return function() {
      var e;
      try {
  ...
  reset: function() {
    this.resetContext();
    this.configuration["default"].reset();
    this.TraceKit.collectWindowErrors = this.configuration["default"].onerror;
    this.configured = false;
    return this;
  },
  install: function() {
    this.TraceKit.collectWindowErrors = this.configuration["default"].onerror;
    this.TraceKit.report.subscribe(this._handleTraceKitSubscription);
    return this;
  },
  _sendRequest: function(data, app) {
    var url;
    app || (app = 'default');
    url = 'http' + ((this.configuration[app].ssl && 's') || '') + '://' + this.configuration[app].host + '/v1/notices.html';
    return this._crossDomainPost(url, data, app);
  },
  _crossDomainPost: function(url, payload, app) {
    ...
    input.type = 'hidden';
    input.name = "api_key";
    input.value = this.configuration[app].api_key;
    form.appendChild(input);
